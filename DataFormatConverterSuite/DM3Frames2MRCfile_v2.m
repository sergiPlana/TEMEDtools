function DM3Frames2MRCfile()
    [filename, pathname] = uigetfile({'*.dm3'}, 'Select your DM3 Frames','MultiSelect','On','L:\Sergi\');
    ...
    bar = waitbar(0,'Initializing ... ','Name','DM3 Frames to MRC File');
    
    name = strcat(pathname,char(filename(1))); 
    slice = getdm3image(name ,1);
    maxOfSFrame = max(max(slice));
    if (maxOfSFrame > 32767)
    ...
    Basic.data='MRC file created by DM3Stack2MRC packer from Sergi Plana Ruiz, Mainz, October 2017';
    ...
    for i=1:Basic.nz
        ...
        elseif (i > 56 && i <= 100)
            Extended.a_tilt(i)=Extended.a_tilt(1)+(i)*1;
        else 
            Extended.a_tilt(i)=Extended.a_tilt(1)+(i+1)*1;
        end
    end
    ...
    msgbox({'DM3 frames are converted to a MRC file into the following path and name:' mrc_name},'DM3 Frames -> MRC')
end