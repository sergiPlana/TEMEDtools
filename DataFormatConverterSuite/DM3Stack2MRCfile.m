function DM3Stack2MRCfile()
    ...
    Basic.amin=min(min(min(slice)));
    fwrite(fid_mrc, Basic.amin, 'float');   % float 4-byte 
    Basic.amax=max(max(max(slice)));
    fwrite(fid_mrc, Basic.amax, 'float');   % float 4-byte 
    Basic.amean=mean(mean(mean(slice)));
    ...
    msgbox({'DM3 stack with the following name:' filename 'is converted into the following MRC file:' mrc_name},'DM3 Stack -> MRC')
end